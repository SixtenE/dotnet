...
- **POST /generate**
  - **Description:** Sends a plain-text prompt to OpenRouter and returns the AI-generated response text.
  - **Headers:**
    - `Content-Type: text/plain`
  - **Request Body:**
    - A plain-text prompt (required), for example:
      ```
      Explain monads in simple terms.
      ```
  - **Response Example (200 OK):**
    - Plain text generated by the configured model.
  - **Error Responses:**
    - `400 Bad Request` if the request body is empty.
    - `500 Internal Server Error` if `OpenRouter:ApiKey` is not configured.

### OpenRouter configuration

Set your OpenRouter API key in `appsettings.json` or via environment variable:

```bash
export OpenRouter__ApiKey="your-openrouter-api-key"
```
